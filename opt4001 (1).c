...
/* -------------------------------------------------------------------------
 * One-shot (forced-mode) support.
 * ---------------------------------------------------------------------- */

opt4001_status_t opt4001_trigger_oneshot(opt4001_t *dev, uint8_t convtime_idx)
{
    /* Clear any stale latched CONVERSION_READY from a previous cycle so we
     * don't mistake it for this conversion's completion. Reading FLAGS clears
     * the latched bit. */
    uint16_t flags;
    (void)read_reg16(dev, OPT4001_REG_FLAGS, &flags);

    /* Forced mode + automatic full-scale range + chosen conversion time.
     * Writing OP_MODE=FORCED starts one conversion; the chip returns to
     * powerdown once it completes. */
    uint16_t ctrl = 0;
    ctrl |= (uint16_t)(OPT4001_CTRL_RANGE_AUTO   << OPT4001_CTRL_RANGE_SHIFT);
    ctrl |= (uint16_t)((convtime_idx & 0x0Fu)    << OPT4001_CTRL_CONVTIME_SHIFT);
    ctrl |= (uint16_t)(OPT4001_OPMODE_FORCED     << OPT4001_CTRL_OPMODE_SHIFT);
    return write_reg16(dev, OPT4001_REG_CTRL, ctrl);
}
                                    ...
                                    void (*delay_ms)(uint32_t))
{
    ...
    for (;;) {
        opt4001_status_t s = read_reg16(dev, OPT4001_REG_FLAGS, &flags);
        if (s != OPT4001_OK) return s;

        if (flags & OPT4001_FLAG_CONV_READY)
            return OPT4001_OK;        /* conversion done */

        if (waited >= timeout_ms)
            return OPT4001_ERR_TIMEOUT;

        if (delay_ms) delay_ms(1);
        waited += 1;                  /* 1 ms per pass if delay_ms sleeps 1 ms */
    }
}
                                      ...
                                      opt4001_result_t *out)
{
    ...
    if ((s = opt4001_trigger_oneshot(dev, convtime_idx)) != OPT4001_OK) return s;
    if ((s = opt4001_wait_ready(dev, timeout_ms, delay_ms)) != OPT4001_OK) return s;
    /* CONVERSION_READY is set: the RESULT registers now hold a fresh sample. */
    return opt4001_read(dev, out);
}

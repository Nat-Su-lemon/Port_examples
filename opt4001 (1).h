...
/* -------------------------------------------------------------------------
 * FLAGS register (0x0C) bit definitions.
 *   [3] OVERLOAD_FLAG           input signal exceeded full-scale
 *   [2] CONVERSION_READY_FLAG   set when a new conversion is complete
 *   [1] FLAG_H                  measurement above high threshold
 *   [0] FLAG_L                  measurement below low threshold
 * Reading FLAGS clears the latched conversion-ready bit.
 * ---------------------------------------------------------------------- */
#define OPT4001_FLAG_L            (1u << 0)
#define OPT4001_FLAG_H            (1u << 1)
#define OPT4001_FLAG_CONV_READY   (1u << 2)
#define OPT4001_FLAG_OVERLOAD     (1u << 3)
...
typedef enum {
    ...
    OPT4001_ERR_TIMEOUT,    /* conversion did not complete in time         */
    OPT4001_ERR_PARAM
} opt4001_status_t;
 ...
 * NOTE: in one-shot/forced mode, only call this AFTER the conversion is done
 * (see opt4001_wait_ready / opt4001_read_oneshot). Reading before the sensor
 * has finished returns a stale or blank sample. */
opt4001_status_t opt4001_read(opt4001_t *dev, opt4001_result_t *out);

/* -------------------------------------------------------------------------
 * One-shot (forced-mode) helpers.
 *
 * Use these when the sensor is powered down between measurements (e.g. one
 * reading per hour). The flow is: trigger a single conversion, wait for the
 * CONVERSION_READY flag, then read. This guarantees you never read a stale or
 * blank result because a measurement is still in progress.
 *
 * `delay_ms` is your platform's millisecond sleep (e.g. HAL_Delay). Pass NULL
 * to busy-spin, but then `timeout_ms` is only an iteration count, not real
 * time — prefer passing a real delay so the timeout is meaningful.
 *
 * Size `timeout_ms` to the conversion time plus margin. A conversion takes
 * roughly the CONV_TIME you selected (e.g. ~800 ms at OPT4001_CONVTIME_800MS,
 * ~100 ms at OPT4001_CONVTIME_100MS), so use ~2x that plus a small cushion.
 * ---------------------------------------------------------------------- */

/* Trigger a single one-shot (forced) conversion, then powerdown. Clears any
 * stale latched conversion-ready flag first. */
opt4001_status_t opt4001_trigger_oneshot(opt4001_t *dev, uint8_t convtime_idx);

/* Poll FLAGS until CONVERSION_READY is set, or `timeout_ms` elapses. */
opt4001_status_t opt4001_wait_ready(opt4001_t *dev, uint32_t timeout_ms,
                                    void (*delay_ms)(uint32_t));

/* Convenience: trigger a one-shot conversion, wait for ready, read + decode. */
opt4001_status_t opt4001_read_oneshot(opt4001_t *dev, uint8_t convtime_idx,
                                      uint32_t timeout_ms,
                                      void (*delay_ms)(uint32_t),
                                      opt4001_result_t *out);
